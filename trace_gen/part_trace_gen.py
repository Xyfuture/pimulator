...
def io_part_trace_gen():
    current_cmd = []
    # 生成需要的通信指令
    # 写到buffer die中的指令
    current_cmd += cmd_score_mvsb[0][0]

    stride = 16
    for k in range(stride):
        current_cmd.append(cmd_score_wrgb[1][16+k])

    with open('./io.trace','w') as f :
        for cmd in current_cmd:
            f.write(cmd+'\n')


def compute_part_trace_gen():
    current_cmd = []
    # 生成需要的计算指令

    stride = 16
    for k in range(stride):
        current_cmd += cmd_score_mac[0][16+k]

    with open('./compute.trace','w') as f :
        for cmd in current_cmd:
            f.write(cmd+'\n')
